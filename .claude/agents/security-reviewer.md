...
## 2. External Process Safety

- Check all `Command::new()` / `spawn()` calls in `src-tauri/src/text_extract.rs`, `src-tauri/src/actions/utils.rs`, `src-tauri/src/actions/handlers.rs`, and `src-tauri/src/commands/apps.rs`
...
- Check all SQLite queries in `src-tauri/src/commands/history.rs`, `src-tauri/src/commands/vectors.rs`, and `src-tauri/src/indexer.rs`
